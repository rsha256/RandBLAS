                            ...
                            Kokkos::View<double**> AB)
{
  ...
  // 1) Create a random pool
  Kokkos::Random_XorShift64_Pool<ExecSpace> pool(12345);

  // 2) Fill columns 0..(n-1) of AB (the "A" part) with normal random data
                           ...
                           Kokkos::View<double*> vals)
{
  ...
  Kokkos::Random_XorShift64_Pool<ExecSpace> pool(9999);
    ...
    KOKKOS_LAMBDA(const int j)
  {
    ...
    for (int i_nz = 0; i_nz < nnz_per_col; i_nz++) {
      // random row in [0..sk_dim)
      int r = (int)(Kokkos::rand<decltype(rand_gen), unsigned int>::draw(rand_gen) % sk_dim);

      // random normal(0,1) value
      double u1 = Kokkos::rand<decltype(rand_gen), double>::draw(rand_gen);
      double u2 = Kokkos::rand<decltype(rand_gen), double>::draw(rand_gen);
